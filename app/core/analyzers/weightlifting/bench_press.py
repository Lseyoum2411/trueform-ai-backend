...
class BenchPressAnalyzer(BaseLiftAnalyzer):
    async def analyze(self, pose_data: List[Dict]) -> AnalysisResult:
        ...
        elbow_score, elbow_metric, elbow_feedback = self.analyze_joint_angles(angles_list, "left_elbow", 90.0, 15.0)
        metrics.append(elbow_metric)
        feedback.extend(elbow_feedback)
        
        shoulder_score, shoulder_metric, shoulder_feedback = self.analyze_joint_angles(angles_list, "left_hip", 45.0, 20.0)
