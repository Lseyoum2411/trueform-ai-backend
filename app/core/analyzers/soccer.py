...
class SoccerAnalyzer(BaseAnalyzer):
    def __init__(self, movement_type: str = "shooting_technique"):
        ...
        if self.movement_type not in ["shooting_technique", "passing_technique", "crossing_technique"]:
    ...
    async def analyze(self, pose_data: List[Dict]) -> AnalysisResult:
        ...
        feedback.append(self.create_feedback(
            "info",
            f"Soccer {self.movement_type.replace('_', ' ')} analysis completed. Detailed analysis coming soon.",
            "general"
        ))
